...
for ark_navn, df_ark in alle_ark.items():
    ...
    if len(alle_omgange) > 0:
        hold_df = pd.DataFrame({
            ...
            'Kører': kører_navne,
            'Stint_Kronologi': stint_kronologi_liste
        })
...
if not sallies_kun.empty:
    with col2:
        ...
        fjern_outliers = st.checkbox("Rens data (Fjern top/bund 5 % omgange)", value=True,
                                     help="Fjerner de 5% hurtigste og langsommeste omgange i HVERT stint.")
    ...
    * **🌟 Overall:** Simpelt gennemsnit af Speed og Consistency.
    ...
    with col_chart:
        ...
        fig_kører, ax_kører = plt.subplots(figsize=(6, 4), dpi=300)
        farver = ['#e74c3c' if x >
